<script>
  ...
  onMount(async () => {
    try {
      ...
      updateIframeParams({ tr: `P12M`,  view: 'ttd', measure: 'total_line_changes', compare_dim: 'author_name' });
    } catch (err) {
      error = err.message;
    } finally {
      isLoading = false;
...
<div class="container">
    <div class="footer">
             It is possible to modify the URL to default to a specific page. In this case, we are defaulting to the Time Dimension detail for the `Total Number of Lines Changed` measure.
